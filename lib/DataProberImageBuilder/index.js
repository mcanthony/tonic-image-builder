var CanvasOffscreenBuffer = require('../CanvasOffscreenBuffer'),
    ...
    PROBE_LINE_X_READY_TOPIC = 'chart-x',
    PROBE_LINE_Y_READY_TOPIC = 'chart-y',
    PROBE_LINE_Z_READY_TOPIC = 'chart-z',
    dataMapping = {
        renderXY: {
            idx: [0,1,2],
...
DataProberImageBuilder.prototype.needsWidget = function(name) {
  return false;
};
...
DataProberImageBuilder.prototype.pushToFront = function(width, height, scaleX, scaleY, lineX, lineY) {
    ...
    if(this.triggerProbeLines) {
        this.emit(PROBE_LINE_X_READY_TOPIC, this.getProbeLine(0));
        this.emit(PROBE_LINE_Y_READY_TOPIC, this.getProbeLine(1));
        this.emit(PROBE_LINE_Z_READY_TOPIC, this.getProbeLine(2));
    }
};
...
DataProberImageBuilder.prototype.onProbeLineXReady = function(callback) {
    return this.on(PROBE_LINE_X_READY_TOPIC, callback);
};

DataProberImageBuilder.prototype.onProbeLineYReady = function(callback) {
    return this.on(PROBE_LINE_Y_READY_TOPIC, callback);
};

DataProberImageBuilder.prototype.onProbeLineZReady = function(callback) {
    return this.on(PROBE_LINE_Z_READY_TOPIC, callback);
};
...
DataProberImageBuilder.prototype.TopicProbeLineXReady = function() {
    return PROBE_LINE_X_READY_TOPIC;
};

DataProberImageBuilder.prototype.TopicProbeLineYReady = function() {
    return PROBE_LINE_Y_READY_TOPIC;
};

DataProberImageBuilder.prototype.TopicProbeLineZReady = function() {
    return PROBE_LINE_Z_READY_TOPIC;
};
