...
export default function WebGlImageBuidler(queryDataModel, pipelineModel, lookupTableManager) {
    ...
    var dataType = queryDataModel.originalData.type;
    if(dataType.indexOf("webgl-light-composite-pipeline") !== -1) {
       this.compositor = new WebGlLightCompositor(queryDataModel, lookupTableManager, this);
       this.controlWidgets = ["LookupTableManagerWidget"].concat(this.controlWidgets)
    } else if(dataType.indexOf("webgl-composite-pipeline") !== -1) {
