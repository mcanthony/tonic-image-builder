    ...
    this.layers.forEach( (layer) => {
        if(layer.active) {
            ...
            for(var i = 0; i < size; i++) {
                var color = lut.getColor(layer.data[size - i - 1]),
                    light = layer.light ? (layer.light[size - i - 1] ? layer.light[size - i - 1] - this.light : 0): 0;
                ...
                if(color[3]) {
                    ...
                    if(layer.hasMesh && layer.meshActive && layer.mesh[size - i - 1]) {
